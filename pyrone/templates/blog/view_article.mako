...
%endif
  ...
  <div class="subtitle">
    <div class="date">${h.timestamp_to_str(request, article.published)}</div>
    <div class="social">${h.get_twitter_share_link_button(request)|n}</div>
    <div class="social">${h.get_facebook_share_button(request, request.url)|n}</div>
